...
if($type=="cumulative_invoice")
{
    ...
    if($mc_id!="")
    {
        $cumulative_order=$prod->get_all_cumulative_orders_order_by_date($mc_id,$invoice_start_date,$invoice_end_date);
    }
    else
    {
...
else
{
	for($i=0;$i<count($cumulative_order);$i++)
	{
        ...
        // $total_price +=($cumulative_order[$i]['o_price']+$o_desc_in_b3['o_price_in_b3']+$o_desc_ex_b5['o_price_ex_b5']);
...
if($order['o_special_agreement_price']==0)
{
    ...
    $total_temp=($total_price + $vat_value);
    
    $total_temp2=number_format($total_temp, 2, ',', '.');

    $invoice.=$total_temp2." ".$currency."</b></span><br><br>";
}
else
{
    $invoice.="&nbsp;<b>";
    $total_temp=$order['o_special_agreement_price']+$vat_value;
...
if($rest>0)
{
	...
	for($z=0;$z<count($language);$z++)
	{
        ...
        $invoice.="<textarea cols='40' rows='3' class='form-control form-control-sm'>".$text."</textarea>";	
        $invoice.=" <br><br>";
...
else
{
	...
	for($z=0;$z<count($language);$z++)
	{
        ...
        if(!empty($licence_taker['IBAN']))
        {
            $invoice.="<textarea name=\"iban_txt\" id=\"iban_txt\" cols='40' rows='3' class='form-control form-control-sm' form=\"create_invoice\">".$licence_taker['Bank_name']." &#13;&#10;".$licence_taker['IBAN']."</textarea>";
...
if($type=="cumulative_invoice")
{
	require('check_cumulative_invoice.php');
}
