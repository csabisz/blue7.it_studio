...
$basement=$prod->xss_fix($_GET['basement']);
$levels_over_ground=$prod->xss_fix($_GET['levels_over_ground']);
$e_length=$prod->xss_fix($_GET['e_length']);
$e_width=$prod->xss_fix($_GET['e_width']);
$environment=$prod->xss_fix($_GET['environment']);
$rs_id=$prod->xss_fix($_GET['rs_id']);
$roof_material=$prod->xss_fix($_GET['roof_material']);
$roof_color=$prod->xss_fix($_GET['roof_color']);
$r_tilt=$prod->xss_fix($_GET['r_tilt']);
$r_kneewall=$prod->xss_fix($_GET['r_kneewall']);
$rop_id=$prod->xss_fix($_GET['rop_id']);
$ww_id=$prod->xss_fix($_GET['ww_id']);
$wlc_id=$prod->xss_fix($_GET['wlc_id']);
$wc_id=$prod->xss_fix($_GET['wc_id']);
$door_shape_sides=$prod->xss_fix($_GET['door_shape_sides']);
$door_texture=$prod->xss_fix($_GET['door_texture']);
$door_color=$prod->xss_fix($_GET['door_color']);
$gc_id=$prod->xss_fix($_GET['gc_id']);
$garage_size=$prod->xss_fix($_GET['garage_size']);
...
if(!empty($basement))
{
	...
	$ex_b5['value']=$basement;
	
	$prod->ajax_update_o_desc_allproducts(json_encode($ex_b5));
...
if(!empty($levels_over_ground))
{
	$ex_b5['o_id']=$o_id;
	$ex_b5['column_name']="levels_over_ground";
	$ex_b5['value']=$levels_over_ground;
...
if(!empty($b5_roof_material))
{
    $all_roof_colors=$domenia2->get_roof_colors($b5_roof_material);

    $result_message="<select id=\"b5_roof_color\" name=\"b5_roof_color\" class=\"form-control form-control-sm\">";
