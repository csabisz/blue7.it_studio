...
for($i=0;$i<count($result_files);$i++)
{
    ...
    <div class="row" id="row<?php echo $result_files[$i]['orf_id']; ?>">
        <div class="col-md-5">
            <input type="text" id="vr_link<?php echo $result_files[$i]['orf_id']; ?>" data-orf_id="<?php echo $result_files[$i]['orf_id']; ?>" class="form-control form-control-sm" value="<?php echo $result_files[$i]['vr_link'];?>" placeholder="Add VR link here">
            <script type="text/javascript">
                $('#vr_link<?php echo $result_files[$i]['orf_id']; ?>').on('focusout',function(){

                    let orf_id=$(this).data('orf_id');
                    let vr_link=$(this).val();

                    $.ajax({
                        url: "<?php echo $base_url;?>ajax/update_vr_link.php",
                        method: "post",
                        data: {
                            orf_id:orf_id,
                            vr_link: vr_link
