...
$save_data['existing_ftl_id'] = $prod->xss_fix($_POST['existing_ftl_id']);
$save_data['orf_id'] =$prod->xss_fix($_POST['orf_id']);
$save_data['rks_id'] = $prod->xss_fix($_POST['room_id']);
...
$fto_content="";
$save_data['fto_content']="";

if(!empty($save_data['existing_ftl_id']))
{
    for($o=0;$o<count($save_data['ft_object']);$o++)
    {
        for($m=0;$m<count($save_data['multiplicator']);$m++)
        {
            if($o==$m)
            {
    ...
    $prod->update_ft_layers(json_encode($save_data));
}
?>