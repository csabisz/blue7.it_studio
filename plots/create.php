...
<section class="top_section">
	...
	{		
		...
		?>
        <p class="w-100 text-center display-4 pt-4"> Add new plot</p>
        ...
        { 
            ...
            $sp7->insert_plot(json_encode($plot_data));

            /*$last_plot_id=$sp7->get_last_plot();

            $o_id=$prod->xss_fix($_POST['o_id']);

            if(!empty($o_id))
            {
                $prod->update_order_plot_id($o_id,$last_plot_id['plot_id']);
            } */
            ?>
            ...
            <br>
            <meta http-equiv="refresh" content="1; url=<?php echo $base_url;?>plots/index.php">
        <?php
        }
        ?>
        <hr class="mb-4">

        <form name="insert_plot_form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <select name="owner_id" class="form-control form-control-sm"> 
                    ...
                    {
                        echo $all_clients[$i]['clientname']." - ".$all_clients[$i]['c_first_name']." ".$all_clients[$i]['c_last_name'];
                    }
                    elseif(!empty($all_clients[$i]['l_first_name']))
                    {
                        echo $all_clients[$i]['clientname']." - ".$all_clients[$i]['l_first_name']." ".$all_clients[$i]['l_last_name'];
                    }
                    else
                    {
