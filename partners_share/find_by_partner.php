...
$partner = $prod->get_user_by_email($_GET['partner']);
$orders = $prod->get_orders_by_daterange($from, $to);

$shares=build_shares_by_partner($orders, $partner);
...
function build_shares_by_partner($orders, $partner2)  {	
	...
	/**/
		...
		foreach ($orders as $order) {
			...
			if(isset($ref_chain[1])) {
				...
				$user = $prod->get_user_by_id($order['u_client_ID']);
				$seller = [];

				if($user) {

					if ( $user['partner_since']!=null && (strtotime($user['partner_since']) < strtotime($order['o_date'])) ) {

						$seller = $user;
					}
					else {

						$seller = $prod->get_user_by_id($user['referrer_id']);
					}
				}
		...
		foreach ($shares as $share_key => $share) {
			
			foreach ($share['generations'] as $gen_key => $generation) {
				
				if($generation['user']['id']!=$partner2['client_ID']) {

					unset($shares[$share_key]['generations'][$gen_key]);
				}
			}

			if(empty($shares[$share_key]['generations'])) {
						...
						<form method="get">
							<div class="form-inline">
								<div class="form-group">
							  	...
							  	<div class="form-group">
									<label class="m-3" for="to">Partner: </label>
									<input list="partners" class="form-control" id="partner" name="partner" autocomplete="off" value="<?=$partner['email']?>">
									<datalist id="partners">
										<?php foreach($partners as $user):?>
							...
							<div class="form-group mt-3 ml-4"><button type="submit" class="btn btn-primary">Set</button></div>
						</form>
					</div>
					<?php $grand_total=[];?>
			        <div id="top_row_total">
									...
									<div class="card-body">
										<div class="row">
											<div class="col">
												<table class="table table-sm">
													<tbody>
														<tr>
															...
															<td>
																<b class="text-primary"><?php echo $share['order']['seller_email']?></b>
															</td>
														</tr>
														<tr>
														...
														<?php foreach($share['generations'] as $generation):?>
															...
															<tr class="striped">
																<th scope="row" colspan="2"><b>Relation <?=$generation['generation'].', '.$generation['percent'].'%, '.$generation['user']['email']?>'s share: </b></th>
												...
												<?php 
												if(array_key_exists($generation['user']['id'], $totals)) {

													array_push($totals[$generation['user']['id']]['totals'], $generation['share']);
												}
												else {

													$totals[$generation['user']['id']]['id']=$generation['user']['id'];
													$totals[$generation['user']['id']]['email']=$generation['user']['email'];
													$totals[$generation['user']['id']]['totals']=[$generation['share']];
												}
												?>
												<?php endforeach;?>
														...
														<?php foreach($totals as $total):?>
																<?php
																	$grand_total[]=array_sum($total['totals']);
																?>
														<?php endforeach;?>
					...
					<div class="row">
						<div class="col">
							...
							<table class="table table-sm">
								<thead>
									<tr>
										<th colspan="4"><h4><b>Total:</b></h4></th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td colspan="4">
											<h5>Partner <a href="#"><?=$partner['email']?></a> for the period of <p class="font-weight-light" style="display:inline"><?=$from.' - '.$to?></p> gets <b><?=number_format(array_sum($grand_total), 2, '.', ' ')?></b> EUR cashback.</h5>
										</td>
									</tr>
								</tbody>
							</table>
