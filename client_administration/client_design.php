...
<section class="top_section">
	...
	{									
		...
		<div class="container-fluid bg-white">
			<!-- <h3>Clients</h3> -->
            <p class="pt-4 w-100 display-4 text-center">Client Design</p>	
            <hr width="450px">
			<?php
			//include('submenu.php');
            ...
            <div class="col" style="width: 44%; float: right" id="checkboxes">
                <div class="float-center">
                    <div id="remove_inactive_block" data-role="fieldcontain" class="checkboxes">
                        <label for="remove_inactive">Don't show inactive clients</label>
                        <input id="remove_inactive" name="remove_inactive" type="checkbox"
                               onclick="remove_inactive()"/>
                    </div>
                </div>

                <div id="only_inactives_block" data-role="fieldcontain" class="checkboxes">
                    <label for="only_inactives">Show only inactive clients</label>
                    <input id="only_inactives" name="only_inactives" type="checkbox"
                           onclick="only_inactives()"/>
                </div>
                ...
                <table class="table table-striped mt-5" style="font-size: 13px; overflow-y: auto;height: 520px; display: block; border-collapse: separate; border-spacing: 0;">
                    ...
                    for($i=0;$i<count($clients);$i++)
                    {
                        ...
                        <tr id="client<?php echo $clients[$i]['client_ID'];?>"
                            data-client-name="<?= (!empty($clients[$i]['c_last_name'])) ? $clients[$i]['c_last_name'] . $clients[$i]['c_first_name'] : $clients[$i]['l_last_name'] . $creators[$i]['l_first_name'] ?>"
                            data-active="<?php if ($clients[$i]['c_status'] === "active") {
                            echo "true";

                        } else {
                            echo "false";
                        } ?>" class="text-center clients <?php
                            if($clients[$i]['c_status'] !="active")
                            {
                                echo "violet";
                            }?>">
                            ...
                            <td class="py-2 px-0">
                                <p class="w-100 mb-0 text-center"><?php $date_time=explode(" ",$clients[$i]['date_registered']);  echo $date_time[0]."".$date_time[1];?></p>  
                            </td>
                            <td class="py-2 px-0">
                            ...
                            <td class="py-2 px-0">
                                <?php echo $clients[$i]['leaders_status']; ?>
                            </td>
                            <td class="py-2 px-0">
                            ...
                            <td class="py-2 px-0">
                                ...
                                <input type="hidden" name="clientid" value="<?php echo $clients[$i]['client_ID'];?>">
                                <a href="presentation_infos.php?clientid=<?php echo $clients[$i]['client_ID'];?>" class="btn btn-sm btn-primary p-0">Modify</a>                                
                            </td>
                        </tr>
                        <?php
                            }
                        ...
                        for($i=0;$i<count($creators);$i++)
                        {
                            ...
                            <tr class="text-center clients <?php
                                ...
                                <td class="py-2 px-0">
                                    <p class="w-100 mb-0 text-center"><?php $date_time=explode(" ",$creators[$i]['date_registered']);  echo $date_time[0]."".$date_time[1];?></p>  
                                </td>
                                <td class="py-2 px-0">
                                ...
                                <td class="py-2 px-0">
                                    <?php echo $creators[$i]['leaders_status']; ?>
                                </td>
                                <td class="py-2 px-0">
                                ...
                                <td class="py-2 px-0">
                                    <form class="w-100 text-center align-self-center" name="delete_client<?php echo $creators[$i]['client_ID'];?>" method="post" action="index.php">				
                                    <input type="hidden" name="clientid" value="<?php echo $creators[$i]['client_ID'];?>">
                                    <a href="presentation_infos.php?clientid=<?php echo $creators[$i]['client_ID'];?>" class="btn btn-sm btn-primary p-0">Modify</a>
                                </td>
                            </tr>
                            <?php
                                }
    ...
    <script>
        ...
        let dont_show_inactive_checkbox = document.getElementById('remove_inactive')
        let show_only_inactive_checkbox = document.getElementById('only_inactives')
        ...
        function only_inactives() {
            ...
            if (show_only_inactive_checkbox.checked) {
                console.log('show only inactives is : Chcecked');
